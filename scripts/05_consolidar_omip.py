...
def main():
    ...
    if not os.path.exists(EXCEL_TEMPLATE):
        raise Exception("❌ No existe OMIP_Template.xlsx")
    ...
    if not os.path.exists(CSV_PATH):
        raise Exception("❌ No existe omip_futuros.csv")

    df = pd.read_csv(CSV_PATH)

    if "TRADE_DATE" not in df.columns:
        raise Exception("❌ CSV sin TRADE_DATE")
    ...
    if not posibles:
        raise Exception("❌ No encuentro columna de precio")
    ...
    # Forward fill SOLO precios (todo el DF son precios)
